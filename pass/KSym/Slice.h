...
class SliceBlock {
  public:
    ...
    typedef typename set<SliceBlock *>::iterator linkIter;
    ...
    // test if bb is a sucessor of item bb, return true if yes
    bool inSTab(SliceBlock *item, BasicBlock *bb) {
      auto i = stab.find(item);
      if(i == stab.end()){
          throw("[Err] item not found in the stab");
      }
      return i->second.find(bb) != i->second.end();
    }
  ...
  protected:
    ...
    // links
    set<SliceBlock *> preds;
    set<SliceBlock *> succs;
...
class Slice {
  public:
    ...
    Slice(Instruction* used, BasicBlock* start , BasicBlock*end,set<BasicBlock*>& pred);
    ~Slice() {
    ...
    typedef typename vector<SliceBlock *>::iterator iterator;
    ...
    set<BasicBlock*>& get_reach(){
        return reach;
    }

  protected:
    // trace defs of `v` back until encounter roots.
    // a root is either a global, alloca or argument,
    // save the roots for latter to follow.
    // Also records the backtraced val in `backs`.
    bool btrace(Value *v);
    // follow users of `v` (usually a root), record
    // the user in `taints` if it is an instruction_ty
    // or a constant_expr.
    void follow(Value *v, set<Value *> &taints);

    SliceBlock *apply(BasicBlock *bb);
    // hists and dedup are used as record tracker during recursion.
    // `dedup` is updated whenever encounter a new basic block,
    // hists is pushed and poped along one path after visit.

    // modify taints and suits.
    // 'suits' add a slice_bb when
    // (1) it is a newly visit BB in scope, and
    // (2) the BB has instructions.
    // `taints` add a terminator (condition var) when
    // (1) it has two successors and
    // (2.1) each succesor leads to different bb in scope(w/o *_SLICE_COMPLETE),
    // (2.2) at least one succesor leads to bb in scope(w/ *_SLICE_COMPLETE),
    // (3) the terminator is not back traced.

    // return the first slice block with tainted instructions.
    SliceBlock *taintCondVar(BasicBlock *bb, list<BasicBlock *> &hists,
        map<BasicBlock *, SliceBlock *> &dedup, set<Value *> &taints);


    void expand(SliceBlock *item);
    void verify();
    bool btraceSpecial(CallInst*);
  ...
  protected:
    ...
    vector<SliceBlock *> suits;
    // for unhandled cases, just abort slicing the current seed and move on to the next
    bool abortCurSeed;
};
...
template class DominatorTreeBase<LLVMSliceBlock,false>;
class SliceDomTree : public DominatorTreeBase<LLVMSliceBlock,false> {
  public:
