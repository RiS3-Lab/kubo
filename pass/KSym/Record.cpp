...
void Record::CFG(Slice &s) {
  errs()<<"root:"<< Helper::getValueName(s.getEntry()->getBlock())<<'\n';
  ...
  for(; i != ie; ++i){
    ...
    errs()<<"  name:"<<Helper::getValueName(ptr->getBlock())<<'\n';
    //l.log("size", ptr->numInsts());

    SliceBlock::linkIter pi = ptr->predBegin(), pe = ptr->predEnd();
    errs()<<"  pred:\n";
    for(; pi != pe; ++pi){
      errs()<<"    "<<Helper::getValueName((*pi)->getBlock())<<'\n';
    }

    SliceBlock::linkIter si = ptr->succBegin(), se = ptr->succEnd();
    errs()<<"  succ:\n";
    for(; si != se; ++si){
      errs()<<"    "<<Helper::getValueName((*si)->getBlock())<<'\n';
    }
  }
}
