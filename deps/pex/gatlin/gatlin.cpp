...
static inline std::string getScopeName(const GlobalValue *GV) {
    if (GlobalValue::isExternalLinkage(GV->getLinkage()))
        return GV->getName();
    else {
        string moduleName =  GV->getParent()->getModuleIdentifier();
        return "_" + moduleName + "/" + GV->getName().str();
    }
}
void gatlin::dump_callees(){
    errs()<<"\n[dumpCallees]\n";
    errs()<< "Num of Callees: " << idcs2callee.size() << "\n";
    int func_id = 0;
    int block_id = 0;
    int inst_id = 0;
    //prepare a mapping from func to its id
    map<Function*,int> func_map;
    for(auto each_func_it = m->begin(); each_func_it != m->end(); each_func_it++){
        Function* func = &*(each_func_it);
        func_map.insert(make_pair(func,func_id));
        func_id ++;
    }
    func_id = 0;
    ...
    for(auto each_func_it = m->begin(); each_func_it != m->end();func_id++, each_func_it++){
        block_id = 0;
        for(auto each_block_it = each_func_it->begin(); each_block_it != each_func_it->end();block_id++, each_block_it ++){
            inst_id = 0;
            for(auto each_inst_it = each_block_it->begin(); each_inst_it != each_block_it->end();inst_id++, each_inst_it++){
                Instruction* inst = &*each_inst_it;
                if(!isa<CallInst>(inst)){
                    continue;
                }
                CallInst* ci = cast<CallInst>(inst);
                if(idcs2callee.find(ci) == idcs2callee.end()){
                    continue;
                }
                auto it = idcs2callee.find(ci);
                const CallInst* CI = dyn_cast<CallInst>(it->first);
                assert(CI!=NULL);
                FunctionSet v = *(it->second);
                // only dump indirect call?
                if (CI->isInlineAsm() || CI->getCalledFunction() /*|| v.empty()*/)
                    continue;

                errs() << "CS:("<< to_string(func_id) <<")(" << to_string(block_id)<<","<< to_string(inst_id) <<")\n";

                // for debugging
                //dbgs()<< "text:"<<*CI;
                //dbgs()<<" id:"<<func_id<<','<<block_id<<','<<inst_id<<'\n';

                errs() << "Callees: ";
                for (FunctionSet::iterator j = v.begin(), ej = v.end();
                     j != ej; ++j) {
                    errs() << func_map.at(*j) << "::";
                }
                if (v.empty()) {
                    errs() << "!!EMPTY =>" << *CI->getCalledValue()<<"\n";
                    errs()<< "Uninitialized function pointer is dereferenced!\n";
                }
                errs() << "\n";
            }
        }
    }
    for (ConstInst2Func::iterator i = idcs2callee.begin(),e = idcs2callee.end(); i != e; ++i) {
    }
    errs()<<"\n[End of dumpCallees]\n";
}

pair<int,int> get_block_inst_id(Function * f,Instruction* inst){
    int block_id = 0;
    int inst_id = 0;
    ...
    for(auto each_block_it = f->begin(); each_block_it != f->end();block_id++, each_block_it ++) {
        ...
        for (auto each_inst_it = each_block_it->begin();
            ...
            if(inst == iter_inst){
                return make_pair(block_id,inst_id);
            }
        }
    }
    assert(false);
}
void gatlin::dump_callers() {
    errs()<<"\n[dumpCallers]\n";
    ...
    for (auto M : f2csi_type0) {
        Function *F = M.first;
        InstructionSet CIS = *(M.second);
        errs()<<"F : " << func_map.at(F) << "\n";
        ...
        for (Instruction * I : CIS) {
            ...
            if (CallerF && CallerF->hasName()) {
                errs()<<"(" << func_map.at(CallerF) << ") ";
            } else {
                errs()<<"(anonymous) ";
            }
            pair<int,int> block_inst_id = get_block_inst_id(CallerF,CI);
            errs()<<'('<<block_inst_id.first<<','<<block_inst_id.second<<')' << "\n";
        }
    }
    for (auto M : f2csi_type1) {
        Function *F = M.first;
        InstructionSet CIS = *(M.second);
        errs()<<"F : " << func_map.at(F) << "\n";
...
FunctionSet gatlin::resolve_indirect_callee_using_kmi(CallInst* ci, int& err)
{
    ...
    for(auto _gep: geps)
    {
        ...
        Type* cvt = dyn_cast<PointerType>(gep->getPointerOperandType())->getElementType();
...
void gatlin::populate_indcall_list_through_kmi(Module& module) {
    ...
    for (auto *idc: idcs) {
        ...
        int err = 6;
        //we resolved type and there's a matching object, buidcs2calleet no fptr defined
...
resolved:
    ...
    if(idcs.size() > 0){
        ...
        errs() << "# pure solved:" << pure_solved << " " << (100 * pure_solved / idcs.size()) << "%\n";
        ...
        "\n";
        errs() << "# of indirect call sites: " << idcs.size() << "\n";
        errs() << "# resolved by KMI:" << count << " " << 0 << "%\n";
        errs() << "#     - KMI:" << kmi_cnt << " " << 0 << "%\n";
        errs() << "#     - DKMI:" << dkmi_cnt << " " << 0 << "%\n";
        errs() << "# (total target) of callee:" << targets << "\n";
        errs() << "# undefined-found-m : " << undefined_1 << " " << 0<< "%\n";
        errs() << "# undefined-udf-m : " << undefined_2 << " " << 0 << "%\n";
        errs() << "# fpara(KMI can not handle, try SVF?): "
               << fpar_cnt
               << " " << 0
               << "%\n";
