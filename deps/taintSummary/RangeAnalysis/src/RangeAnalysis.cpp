...
Range Range::Or(const Range &other) const {
  ...
  int64_t signedC;
  int64_t signedD;
  int64_t signedMax = APInt::getSignedMaxValue(64).getSExtValue();
  int lenA = a.getMinSignedBits();
  int lenB = b.getMinSignedBits();
  int lenC = c.getMinSignedBits();
  int lenD = d.getMinSignedBits();

  if(lenA >= 64)
      signedA = signedMax;
  else
      signedA = a.getSExtValue();
  if(lenB >= 64)
      signedB = signedMax;
  else
      signedB = b.getSExtValue();
  if(lenC >= 64)
      signedC = signedMax;
  else
      signedC = c.getSExtValue();
  if(lenD >= 64)
      signedD = signedMax;
  else
      signedD = d.getSExtValue();
  switch (switchval) {
  case 0:
    l = minOR(signedA, signedB, signedC,
              signedD);
    u = maxOR(signedA, signedB, signedC,
              signedD);
    break;
  case 1:
    l = a;
  ...
  case 5:
    l = (a.slt(c) ? a : c);
    u = maxOR(0, signedB, 0, signedD);
    break;
  case 7:
    l = minOR(signedA, 0xFFFFFFFF, signedC, signedD);
    u = minOR(0, signedB, signedC, signedD);
    break;
  case 12:
    l = minOR(signedA, signedB, signedC,
              signedD);
    ...
    break;
  case 13:
    l = minOR(signedA, signedB, signedC, 0xFFFFFFFF);
    u = maxOR(signedA, signedB, 0, signedD);
    break;
  case 15:
    l = minOR(signedA, signedB, signedC,
              signedD);
