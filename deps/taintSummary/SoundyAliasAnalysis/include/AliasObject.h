...
//#define DEBUG_FETCH_POINTS_TO_OBJECTS
    ...
    public:
        ...
        long fieldId;
        // field id of the destination object to which this pointer points to
        long dstfieldId;
    ...
    public:
        ...
        ~AliasObject() {
            ...
            initializingInstructions.clear();
        }
        ...
        void performStrongUpdate(long srcfieldId, std::set<PointerPointsTo*>* dstPointsTo, Instruction *propogatingInstr) {
            ...
            auto it = this->pointsTo.begin();
            while(it != this->pointsTo.end()){
                if((*it)->fieldId == srcfieldId) {
                    //delete(*it);
                    it = this->pointsTo.erase(it);
                }else{
                    tmpCopy.push_back(*it);
                    it++;
                }
            }
...
*/
        ...
        void fetchPointsToObjects(long srcfieldId, std::set<std::pair<long, AliasObject*>> &dstObjects,
            ...
            if(!hasObjects && (create_arg_obj || this->isFunctionArg())) {
                ...
                dbgs() << "\n";
#endif
                AliasObject *newObj = this->makeCopy(); // so a alias object pointsTO itself?
    ...
    public:
        ...
        FunctionLocalVariable(AllocaInst &targetInst, std::vector<std::pair<Instruction *,Function*> > *callSites) {
    ...
    public:
        ...
        HeapLocation(Instruction &allocSite, Type* targetType, std::vector<std::pair<Instruction *,Function*> > *callSites,
    ...
    public:
        ...
        FunctionArgument(Value *targetArgument, Type* targetType, Function *tarFunction, std::vector<std::pair<Instruction *,Function*> > *callSites) {
            this->targetType = targetType;
