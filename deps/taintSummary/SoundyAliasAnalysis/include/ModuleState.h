...
//#define DEBUG_GLOBALS
    ...
    public:
        void printContext(llvm::raw_ostream& O) {
            ...
            bool putComma = false;
            //std::string str;
            for(auto callSiteIter = callSites->begin(); callSiteIter != callSites->end();callSites++) {
                Instruction * currCallSite = callSiteIter->first;
                //O << "   ";
                if(putComma) {
                    O << ",";
                }
        ...
        ~AnalysisContext(){
            callSites->clear();
            callSites->shrink_to_fit();
            delete(callSites);
        }
        AnalysisContext(){

        }
        std::vector<std::pair<Instruction *,Function*> > *callSites;
    };
    ...
    public:
        ...
        void cleanup() {
            // clean up
            // lcm comments:
            // it's messing up, instead of cleaning up
            std::set<AliasObject*> deletedObjects;
            std::set<Value*> globalValues;
            std::set<ObjectPointsTo*> deletedObjectsPointTo;
            // all global variables.
            for(auto glob_iter = globalVariables.begin(); glob_iter != globalVariables.end(); glob_iter++) {
                globalValues.insert(glob_iter->first);
                ...
                for(auto currPointsTo: *targetPointsTo) {
                    ...
                    if(deletedObjects.find(targetAliasObj) == deletedObjects.end()) {
                        ...
                        for(auto objIt = targetAliasObj->pointsTo.begin();objIt != targetAliasObj->pointsTo.end(); objIt++){
                            deletedObjectsPointTo.insert(*objIt);
                        }
                        delete(targetAliasObj);
                    }
                    deletedObjectsPointTo.insert(currPointsTo);
                    delete(currPointsTo);
                }
                targetPointsTo->clear();
            ...
            for(auto ptInfo = pointToInformation.begin(); ptInfo != pointToInformation.end(); ptInfo++) {
                ...
                for(auto pointsTo_iter = ptInfo->second->begin(); pointsTo_iter != ptInfo->second->end();pointsTo_iter++) {
                    Value * v = pointsTo_iter->first;
                    if(globalValues.find(v) != globalValues.end())
                        continue;
                    auto targetPointsTo = pointsTo_iter->second;
                    for(auto currPointsTo: *targetPointsTo) {
                        auto targetAliasObj = currPointsTo->targetObject;
                        if(deletedObjects.find(targetAliasObj) == deletedObjects.end() and targetAliasObj != nullptr) {
                            deletedObjects.insert(targetAliasObj);
                            bool hasAMemberWhichWasDeleted = false;
                            for(auto objIt = targetAliasObj->pointsTo.begin();objIt != targetAliasObj->pointsTo.end(); objIt++){
                                if(deletedObjectsPointTo.find(*objIt) != deletedObjectsPointTo.end())
                                    hasAMemberWhichWasDeleted = true;
                                else{
                                    deletedObjectsPointTo.insert(*objIt);
                                }
                            }
                            if(!hasAMemberWhichWasDeleted){
                                // you must be kidding me
                                // this is the most ridiculous destructor I've ever seen
                                delete(targetAliasObj);
                            }
                        }
                        deletedObjectsPointTo.insert(currPointsTo);
                ...
                ptInfo->second->clear();
                delete(ptInfo->second);
            }
            pointToInformation.clear();



            for(auto each_func_block_set = loopExitBlocks.begin(); each_func_block_set != loopExitBlocks.end();each_func_block_set++){
                delete(each_func_block_set->second);
            }
            loopExitBlocks.clear();
            for(auto each_taint = taintInformation.begin(); each_taint != taintInformation.end();each_taint++){
                for(auto each_var_taint = each_taint->second->begin();each_var_taint != each_taint->second->end();each_var_taint++){
                    for(auto eachTaintFlag = each_var_taint->second->begin();eachTaintFlag != each_var_taint->second->end();eachTaintFlag++){
                        delete(*eachTaintFlag);
                    }
                    delete(each_var_taint->second);
                }
                delete(each_taint->second);
            }

            for(auto each_context = availableAnalysisContexts.begin(); each_context != availableAnalysisContexts.end(); each_context++){
                delete(*each_context);
            }
            availableAnalysisContexts.clear();
        }
        ...
        AnalysisContext* getContext(std::vector<std::pair<Instruction*,Function*> > *callSites) {
         ...
         *
         */
        void getOrCreateContext(std::vector<std::pair<Instruction *,Function*> > *callSites, std::map<Value *,
        ...
        void insertPointsTo(std::vector<std::pair<Instruction*,Function*> > *callSites, std::map<Value *, std::set<PointerPointsTo*>*> *targetInfo) {
            AnalysisContext* currContext = getContext(callSites);
         ...
         */
        std::map<Value *, std::set<PointerPointsTo*>*>* getPointsToInfo(std::vector<std::pair<Instruction *,Function*> > *callSites) {
            AnalysisContext* currContext = getContext(callSites);
         ...
         */
        std::map<Value *, std::set<TaintFlag*>*>* getTaintInfo(std::vector<std::pair<Instruction*,Function*> > *callSites) {
            AnalysisContext* currContext = getContext(callSites);
