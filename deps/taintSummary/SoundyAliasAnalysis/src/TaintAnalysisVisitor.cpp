...
//#define DEBUG_CALL_INSTR
        ...
        for(PointerPointsTo *currPtTo: currValPointsTo) {
            ...
            if(currTaintSet != nullptr) {
                for(auto a: *currTaintSet) {
                    if(std::find_if(retTaintFlag.begin(), retTaintFlag.end(), [a](const TaintFlag *n) {return  n->isTaintEquals(a);}) == retTaintFlag.end()) {
        ...
        if(srcTaintInfo != nullptr) {
            ...
            for (auto currTaint:*srcTaintInfo) {
                ...
                if(add_taint) {
                    ...
                    newTaintFlag->addToTrace(targetInstruction);
    ...
    std::set<TaintFlag*>* TaintAnalysisVisitor::mergeTaintInfo(std::set<Value *> &srcVals, Value *targetInstr) {
        ...
        for(auto currVal:srcVals) {
            ...
            if(currValTaintInfo != nullptr) {
                this->makeTaintInfoCopy(targetInstr, dyn_cast<Instruction>(targetInstr),currValTaintInfo, newTaintInfo);
            }
        }
    ...
    void TaintAnalysisVisitor::propogateTaintToArguments(std::set<long> &taintedArgs, CallInst &I) {
        ...
        for(auto currArgNo: taintedArgs) {
            int numArg = I.getNumArgOperands();
            if(numArg <= currArgNo)
                continue;
    ...
    std::vector<int> TaintAnalysisVisitor::setupCallContext(CallInst &I, Function *currFunction,
                                                std::vector<std::pair<Instruction *,Function*> > *newCallContext) {

        std::map<Value *, std::set<TaintFlag*>*> *contextTaintInfo = currState.getTaintInfo(newCallContext);
        unsigned int arg_no = 0;
        std::vector<int> taint_arg_idx;
                ...
                for(Function::arg_iterator farg_begin = currFunction->arg_begin(), farg_end = currFunction->arg_end();
                    ...
                    if(farg_no == arg_no) {
                        ...
                        if(currArgTaintInfo == nullptr){
                            errs()<<"ERR:src taint is not empty while dst is empty\n";
                            break;
                        }
#ifdef DEBUG_CALL_INSTR
...
#endif
                        bool is_taint_arg = false;
                        for(auto taint_it = currArgTaintInfo->begin(); taint_it != currArgTaintInfo->end();taint_it++){
                            if((*taint_it)->isTainted())
                                is_taint_arg = true;
                        }
                        if(is_taint_arg){
                            taint_arg_idx.push_back(farg_no);
                        }
...
#endif
        // we do not need avisitICmpInstny special taint handling..because alias takes care of propagating
...
#define DEBUG_CALL_INSTR
          ...
          // This is an atoi like function?
           // if yes, get the taint of the object pointed by the first argument.
            ...
            if(!allPointerTaint.empty()) {
                std::set<TaintFlag *> *newTaintSet = this->makeTaintInfoCopy(I.getArgOperand(0), &I, &allPointerTaint);
    ...
    VisitorCallback* TaintAnalysisVisitor::visitCallInst(CallInst &I, Function *currFunc,
                                                         ...
                                                         std::vector<std::pair<Instruction *,Function*> > *callSiteContext,
                                                         std::map<Function*,std::vector<int> > * globalTaint,std::vector<Function*>* targetFuncs,FunctionTaintSummary*summary) {
        ...
        std::vector<int> taint_args = setupCallContext(I, currFunc, callSiteContext);
        bool anything_new = false;

        std::map<Value *, std::set<TaintFlag*>*> *contextTaintInfo = currState.getTaintInfo(callSiteContext);
        int farg_no = 0;
        bool arg_is_taint = false;
        for(Function::arg_iterator farg_begin = currFunc->arg_begin(), farg_end = currFunc->arg_end();farg_begin != farg_end; farg_begin++,farg_no++) {
            Value *currfArgVal = &(*farg_begin);
            std::set<TaintFlag*> taint_set;
            std::set<PointerPointsTo*> *currPtsTo = PointsToUtils::getPointsToObjects(this->currState, callSiteContext, currfArgVal);
            if(currPtsTo != nullptr){
                for(PointerPointsTo *currPtTo: *currPtsTo) {
                    std::set<TaintFlag*> *currTaintSet = currPtTo->targetObject->getFieldTaintInfo(currPtTo->dstfieldId);
                    if(currTaintSet != nullptr)
                        taint_set.insert(currTaintSet->begin(),currTaintSet->end());
                }
            }

            std::map<Value *, std::set<TaintFlag*>*>::iterator taint_info_it =  contextTaintInfo->find(currfArgVal);

            if( taint_info_it != contextTaintInfo->end()){
                std::set<TaintFlag*>* taint_set_from_taint = taint_info_it->second;
                if(!taint_set_from_taint->empty())
                    taint_set.insert(taint_set_from_taint->begin(),taint_set_from_taint->end());
            }
            for(auto eachTaint = taint_set.begin() ; eachTaint != taint_set.end(); eachTaint++){
                if((*eachTaint)->isTainted()){
                    std::stringstream s;
                    std::string arg_name;
                    raw_string_ostream rso(arg_name);
                    currfArgVal->print(rso);
                    s<<"target taint arg:"<<farg_no<<'|'<<arg_name<<"\n";

                    Value* taintSource = (*eachTaint)->getInstructionTrace()->front();
                    int taintArg = -1;
                    int index = 0;
                    ...
                    if(taintArg == -1){
                        // then it must come from fetch
                        CallInst* ci = cast<CallInst>(taintSource);
                        ...
                        assert(isTaintInitiator);
                    }
                    summary->insertTaintMap(&I,farg_no,taintSource,taintArg);
                }
            }
            summary->insertCallee(&I,currFunc);
        }
    ...
    void TaintAnalysisVisitor::printTaintResults(llvm::raw_ostream& O) const {
        /*
         * Dump all the alias analysis result information into provided stream.
         */
        std::map<Value *, std::set< TaintFlag*>*>*  taintMap = this->currState.getTaintInfo(this->currFuncCallSites);
        for(auto ai:*taintMap) {
            O << "\nValue:";
            ai.first->print(O);
            O << " has following taint info\n";
