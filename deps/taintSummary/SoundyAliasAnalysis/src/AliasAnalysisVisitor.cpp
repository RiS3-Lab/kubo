...
//#define MAX_ALIAS_OBJ 50
    ...
    void AliasAnalysisVisitor::updatePointsToObjects(Value *srcPointer, std::set<PointerPointsTo*>* newPointsToInfo) {
        ...
        if(prevPointsToSet != targetPointsToMap->end()) {
            ...
            for(PointerPointsTo *currPointsTo: *newPointsToInfo) {
                ...
                auto oldPointTo = std::find_if(existingPointsTo->begin(), existingPointsTo->end(), [currPointsTo](const PointerPointsTo *n) {return  n->isIdenticalPointsTo(currPointsTo);});
                if(oldPointTo == existingPointsTo->end()) {
                    ...
                    /*
                    auto oldPoint =(*oldPointTo)->targetObject->pointsTo;
                    auto newPoint = currPointsTo->targetObject->pointsTo;
                    auto oldIt = oldPoint.begin();
                    while(oldIt!=oldPoint.end()){
                        if(std::find(newPoint.begin(),newPoint.end(),*oldIt) == newPoint.end())
                            oldIt = oldPoint.erase(oldIt);
                        else
                            oldIt ++;
                    }*/
                    delete (currPointsTo);
    ...
    void AliasAnalysisVisitor::visitLoadInst(LoadInst &I) {
        ...
        // this set stores the <fieldid, targetobject> of all the objects to which the srcPointer points.
        std::set<std::pair<long, AliasObject*>> targetObjects;
        for(PointerPointsTo *currPointsToObj:*srcPointsTo) {
        ...
        for(const std::pair<long, AliasObject*> &currObjPair:targetObjects) {
            ...
            if(finalObjects.size()>=targetObjects.size())
                break;
        }
    ...
    void AliasAnalysisVisitor::visitVACopyInst(VACopyInst &I) {
        //TODO handle this intrinsic
        return;
    }

    void AliasAnalysisVisitor::setupCallContext(CallInst &I, Function *currFunction, std::vector<std::pair<Instruction *,Function*> > *newCallContext) {
    ...
    VisitorCallback* AliasAnalysisVisitor::visitCallInst(CallInst &I, Function *currFunc,
                                                         ...
                                                         std::map<Function*,std::vector<int> > * globalTaintInfo,std::vector<Function*>* targetFuncs,FunctionTaintSummary*summary) {
        ...
        if(currFunc->isDeclaration()) {
            ...
            if(targetChecker->is_memcpy_function(currFunc)) {
                ...
                bool is_handled;
                is_handled = false;
                std::set<PointerPointsTo *> *newPointsToInfo = (std::set<PointerPointsTo *> *) ((AliasAnalysisVisitor::functionHandler)->handleFunction(I, currFunc,(void *) (this->currFuncCallSites),
    ...
    void AliasAnalysisVisitor::printAliasAnalysisResults(llvm::raw_ostream& O) const {
        ...
        for(auto ai:*targetPointsToMap) {
            ...
            O << "\nPointer:";
            //ai.first->print(O);
            if(Function * f = dyn_cast<Function>(ai.first)){
                O<<"Func:"<<f->getName().str()<<'\n';
            }else{
                ai.first->print(O);
            }
        ...
        dbgs()<<"global:"<<globalNum<<"|nonGlobal:"<<nonGlobal<<'\n';
    }
}
