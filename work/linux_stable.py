...
class APP_LINUX_STABLE(App):
    ...
    def build_impl(self, psrc, pobj, pbin, pext, plog):
        with cd(pobj):
            cmd = "make V=1 -j8 vmlinux modules"
    ...
    def group_line_after_5_4(self,line):
        line = line.strip()
        if not "ar cDPrST" in line:
            return (False,None)
        if not line.startswith("rm -f "):
            return (False,None)
        argv = shellsplit(line.split(";")[1].strip())
    ...
    def group_line(self, line):
        ...
        if int(toks[0]) < 4:
            ...
            if int(toks[1]) < 4:
                ...
                return self.group_line_after_5_4(line)
        else:
            assert(False)
